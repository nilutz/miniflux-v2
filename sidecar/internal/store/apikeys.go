...
// (task 17) and returns the id of the user that owns it. It is the ONLY
// thing that makes an HTTP caller "who they say they are" from the
// sidecar's point of view -- see internal/web's auth middleware, which
// derives every protected request's search scope from this return value
// rather than from anything the caller separately asserts (a "user" query
// parameter, say).
//
...
// itself deleted. Both cases must look identical to a caller: § Behaviour
// in this task's brief requires "do not leak whether a token exists",
// which is exactly what collapsing "unknown" and "revoked" into the same
// (0, false, nil) return does.
//
...
// the fork's own REST API, which is the whole point of task 17: one
// Miniflux key works against both services.
//
...
// first time, which this task's brief explicitly rules out. A key that is
// only ever used against the sidecar therefore never advances its
// last_used_at in Miniflux's own UI -- an accepted, documented trade-off
// (see sidecar/README.md), not an oversight.
